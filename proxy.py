...
async def chat_completions(request: Request):
    ...
    try:
        ...
        if not messages or not isinstance(messages, list):
            raise HTTPException(status_code=400, detail="Messages field must be a non-empty array")
        
        # 处理消息历史
        prompt = process_messages(messages)
        if not prompt:
            raise HTTPException(status_code=400, detail="Failed to process messages into prompt")
        
        logging.debug(f"[REQUEST_PARSED] id={request_id} model={model} message_count={len(messages)}")
        ...
        logging.debug(f"[RESPONSE_READY] id={request_id}")
        return completion
        
    except Exception as e:
        logging.error(f"[REQUEST_ERROR] id={request_id} error={str(e)}")
        raise HTTPException(
            status_code=400,
            detail=str(e)
        )
    
    logging.debug(f"[PROMPT_READY] id={request_id} prompt={prompt[:100]}...")
    
    # prepare gemini CLI call
    gemini_path = os.getenv('GEMINI_PATH', '/opt/homebrew/bin/gemini')
    logging.debug(f"[GEMINI_PATH] id={request_id} path={gemini_path}")
    
    try:
        # 检查 Gemini CLI 是否存在
        if not os.path.exists(gemini_path):
            logging.error(f"[GEMINI_ERROR] id={request_id} error=CLI not found at {gemini_path}")
            raise HTTPException(status_code=500, detail=f"Gemini CLI not found at {gemini_path}")
        
        logging.debug(f"[GEMINI_START] id={request_id} executing command...")
        # 运行带有提示参数的 Gemini CLI（使用非交互模式）
        proc = await asyncio.create_subprocess_exec(
            gemini_path,
            '--prompt', prompt,
            '--yolo',  # 自动接受所有操作
            '--approval-mode', 'yolo',  # 自动批准所有工具操作
        ...
        try:
            logging.debug(f"[GEMINI_EXECUTE] id={request_id} waiting for response...")
            stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=30.0)  # 30秒超时
            logging.debug(f"[GEMINI_RESPONSE] id={request_id} received response")
        except asyncio.TimeoutError:
            logging.error(f"[GEMINI_TIMEOUT] id={request_id} timeout after 30s")
            if proc:
                proc.terminate()
                await proc.wait()
            raise HTTPException(status_code=504, detail="Gemini CLI response timed out")
            
        if proc.returncode != 0:
            error_msg = stderr.decode() if stderr else "Unknown error"
            logging.error(f"[GEMINI_ERROR] id={request_id} code={proc.returncode} error={error_msg}")
            raise HTTPException(status_code=500, detail=f"Gemini CLI failed: {error_msg}")
        
        raw_output = stdout.decode()
        logging.debug(f"[GEMINI_SUCCESS] id={request_id} output_length={len(raw_output)}")
        
        # 处理响应
        response = validate_and_normalize_response(raw_output, request_id)
        ...
        completion = {
            ...
            "object": "chat.completion",
            "created": int(time.time()),
            "model": model or "gemini-2.5-pro-preview-06-05",
            "system_fingerprint": "fp_" + request_id[:8],
            "choices": [
                {
                    "index": 0,
                    "message": {
        ...
        # 记录详细的响应内容
        logging.info(f"[RESPONSE_DEBUG] id={request_id} response_json={json.dumps(completion, ensure_ascii=False)}")
        
        return completion
            
    except Exception as e:
        logging.error(f"[GEMINI_ERROR] id={request_id} error={str(e)}")
        error_response = {
            "error": {
...
async def run_gemini(prompt: str, request_id: str) -> str:
    """Run Gemini CLI with the given prompt."""
    print(f"[DEBUG] Running Gemini with prompt: {prompt[:100]}")
    gemini_path = os.getenv('GEMINI_PATH', '/opt/homebrew/bin/gemini')
    ...
    try:
        ...
        print(f"[DEBUG] Started Gemini process with ID: {proc.pid}")
        
        stdout, stderr = await proc.communicate()
        print(f"[DEBUG] Gemini process completed with return code: {proc.returncode}")
        
        if stderr:
            print(f"[DEBUG] Gemini stderr: {stderr.decode()}")
        
        if stdout:
            print(f"[DEBUG] Gemini raw output: {stdout.decode()[:200]}")
            
        return stdout.decode() if stdout else ""
    except Exception as e:
        print(f"[ERROR] Failed to run Gemini: {str(e)}")
...
if __name__ == "__main__":
    ...
    async def handle_chat_completions(request):
        try:
            ...
            # Run Gemini
            raw_output = await run_gemini(prompt, request_id)
            print(f"[DEBUG] Raw output received: {raw_output[:200]}")
